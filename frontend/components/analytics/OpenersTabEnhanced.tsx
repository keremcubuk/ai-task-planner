...
export function OpenersTabEnhanced({
  byOpenedBy,
  byBucketCategory,
  openerComments,
  onCommentChange,
  ...
  const openedByEntries = Object.entries(byOpenedBy)
    ...
    .filter(([opener]) => 
      opener.toLowerCase().includes(searchTerm.toLowerCase())
    )
    .sort((a, b) => {
      switch (sortBy) {
        case 'completion':
      ...
      <div className="bg-blue-50 border border-blue-200 rounded-lg p-4">
        ...
        {showExplanation && (
          <div className="mt-4 space-y-3 text-sm text-blue-900">
            ...
            <div>
              <strong>💎 Kalite Skoru:</strong> Açılan ticketların ne kadarının geçerli olduğu (100 - Declined%). Yüksek skor = Az declined, kaliteli issue açıyor.
            </div>
            <div>
              ...
              {openedByEntries.map(([openedBy, stats]) => (
                <tr key={openedBy} className="hover:bg-gray-50">
                  ...
                  <td className="px-3 py-3 whitespace-nowrap text-sm">
                    <div className="flex items-center gap-2">
                      <div className="w-16 bg-gray-200 rounded-full h-2">
                        <div className="bg-indigo-600 h-2 rounded-full" style={{ width: `${stats.qualityScore}%` }} />
                      </div>
                      <span className="text-indigo-600 font-medium">{stats.qualityScore}%</span>
                    </div>
                  </td>
          ...
          <div key={openedBy} className="bg-white p-6 rounded-lg shadow">
            ...
            <div className="grid grid-cols-3 gap-3 mb-4">
              ...
              <div className="text-center">
                ...
                <div className="text-xl font-bold text-indigo-600">{stats.qualityScore}%</div>
              </div>
              <div className="text-center">
            ...
            <div className="space-y-4">
              {stats.stuckComponents.filter(item => item.component.toLowerCase() !== 'unknown').length > 0 && (
                <div>
                  <h4 className="text-sm font-medium text-red-600 mb-2">⚠️ Takıldığı Componentler</h4>
                  <div className="space-y-2">
                    {stats.stuckComponents.filter(item => item.component.toLowerCase() !== 'unknown').map((item) => (
                      <div 
                        key={item.component} 
              ...
              {stats.solvedInProjectComponents.filter(item => item.component.toLowerCase() !== 'unknown').length > 0 && (
                <div>
                  <h4 className="text-sm font-medium text-green-600 mb-2">✅ Projede Çözülen Componentler</h4>
                  <div className="space-y-2">
                    {stats.solvedInProjectComponents.filter(item => item.component.toLowerCase() !== 'unknown').map((item) => (
                      <div 
                        key={item.component} 
              ...
              {stats.topComponents.filter(item => item.component.toLowerCase() !== 'unknown').length > 0 && (
                <div>
                  <h4 className="text-sm font-medium text-blue-600 mb-2">📊 En Çok Açtığı Componentler</h4>
                  <div className="space-y-1">
                    {stats.topComponents.filter(item => item.component.toLowerCase() !== 'unknown').slice(0, 5).map((item, idx) => (
                      <div 
                        key={item.component} 
